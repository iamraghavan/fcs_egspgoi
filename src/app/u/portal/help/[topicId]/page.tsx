...
import { LayoutDashboard, Award, FileText, MessageSquareWarning, ShieldCheck, User, AlertTriangle, Info, CheckCircle, Fingerprint, GanttChart, BarChart3, BotMessageSquare } from 'lucide-react';
  ...
  'admin-user-management': {
    ...
    details: (
      <div className="space-y-6">
        <Card>
          <CardHeader>
            <CardTitle>Managing User Accounts</CardTitle>
            <CardDescription>The "Faculty Accounts" page is your central hub for managing all users in the system.</CardDescription>
          </CardHeader>
          <CardContent>
            <Accordion type="single" collapsible className="w-full">
              <AccordionItem value="item-1">
                <AccordionTrigger>Creating a Single Account</AccordionTrigger>
                <AccordionContent>
                  Use the "Create New Account" form at the bottom of the page. You will need to provide the user's name, email, a temporary password, and assign their role (Faculty or Office Assistant). For faculty, you must also specify their college and department.
                </AccordionContent>
              </AccordionItem>
              <AccordionItem value="item-2">
                <AccordionTrigger>Viewing and Filtering Users</AccordionTrigger>
                <AccordionContent>
                  The main table lists all users. You can use the search and filter controls at the top to quickly find specific users based on their name, email, college, department, or account status (Active/Inactive).
                </AccordionContent>
              </AccordionItem>
              <AccordionItem value="item-3">
                <AccordionTrigger>Sharing a Profile via WhatsApp</AccordionTrigger>
                <AccordionContent>
                  Click the "View" (eye) icon on any user to open their details. Inside the dialog, you'll find a "Share on WhatsApp" button. This generates a unique, shortened link to the faculty's dashboard, which you can share to highlight their performance.
                </AccordionContent>
              </AccordionItem>
            </Accordion>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>To create many accounts at once, use the "Bulk Import" feature.</CardDescription>
          </CardHeader>
          <CardContent>
            <h4 className="font-semibold mb-2">Step-by-Step Guide:</h4>
            <ol className="list-decimal pl-5 space-y-2 text-muted-foreground">
              <li>Navigate to the "Bulk Import" page from the admin sidebar.</li>
              <li><strong>Download the Template:</strong> Click the link to download the required Excel (`.xlsx`) template. This ensures your data is in the correct format.</li>
              <li><strong>Fill the Template:</strong> Open the template and fill in the details for each user: `name`, `email`, `password`, `role`, `college`, and `department`.</li>
              <li><strong>Upload the File:</strong> Drag and drop your completed file or click to select it. A preview of the first few rows will be shown.</li>
              <li><strong>Process & Import:</strong> Click "Process & Import Users." The system will validate and create an account for each row. A summary of successful and failed imports will be displayed.</li>
            </ol>
          </CardContent>
        </Card>
      </div>
    ...
    details: (
      <div className="space-y-6">
        <Accordion type="multiple" className="w-full space-y-4">
          <AccordionItem value="item-1" className="p-4 border rounded-lg bg-card">
            <AccordionTrigger>Managing Credit Titles</AccordionTrigger>
            <AccordionContent>
              <p className="mb-4">Found under "Credit Titles", this is where you define the templates for all credit activities. A well-defined list is essential for consistency.</p>
              <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
                <li>Create new titles for both positive achievements and negative remarks.</li>
                <li>Assign default point values and descriptions.</li>
                <li>These titles will appear as options for faculty when submitting work and for admins/OAs when issuing credits/remarks.</li>
              </ul>
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="item-2" className="p-4 border rounded-lg bg-card">
            <AccordionTrigger>Reviewing Faculty Submissions</AccordionTrigger>
            <AccordionContent>
              <p className="mb-4">The "Submissions" page lists all "Good Works" submitted by faculty that are pending review.</p>
              <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
                <li>Select a submission from the list to see its details, including notes and attached proof.</li>
                <li>Use the "Approve" or "Reject" buttons to process the submission.</li>
                <li>Use the extensive filters to narrow down submissions by faculty, date, points, and more.</li>
              </ul>
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="item-3" className="p-4 border rounded-lg bg-card">
            <AccordionTrigger>Issuing Direct Positive Credits</AccordionTrigger>
            <AccordionContent>
              <p className="mb-4">Use the "Positive Credits" page to directly award credits to a faculty member without them needing to submit it themselves.</p>
              <p className="text-muted-foreground">This is useful for institutional awards or activities that are tracked centrally. The process is similar to issuing a negative remark: select the faculty, choose a template or enter custom details, and submit.</p>
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="item-4" className="p-4 border rounded-lg bg-card">
            <AccordionTrigger>Issuing Negative Remarks</AccordionTrigger>
            <AccordionContent>
              <p className="mb-4">The "Negative Remarks" page allows you to issue credit deductions for disciplinary actions or failure to comply with duties.</p>
               <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
                <li>Select the faculty member, choose a remark template or enter custom details.</li>
                <li>Provide clear notes and attach proof if necessary.</li>
                <li>Upon submission, the faculty member is notified and can view the remark on their dashboard.</li>
              </ul>
            </AccordionContent>
          </AccordionItem>
        </Accordion>
  ...
  'admin-appeals-reports': {
    ...
    details: (
      <div className="space-y-6">
        <Card>
          <CardHeader>
            <CardTitle>Reviewing Appeals</CardTitle>
            <CardDescription>The "Appeals" page lists all appeals filed by faculty against negative remarks.</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="mb-4">Your role is to provide a fair review of the faculty member's dispute.</p>
             <ul className="list-disc pl-5 space-y-2 text-muted-foreground">
              <li>Select an appeal to view the faculty's reason, the original remark details, and any evidence provided by both sides.</li>
              <li>You can then choose to "Accept" the appeal (which typically reverses the credit deduction) or "Reject" it.</li>
              <li>Provide a clear rationale for your decision in the comments box.</li>
            </ul>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Generating Reports</CardTitle>
            <CardDescription>The "Reports" page provides tools to analyze system-wide data.</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-muted-foreground">Configure the report by selecting a date range and report type (e.g., Credit Distribution by Department) and click "Generate Report" to visualize the data.</p>
          </CardContent>
        </Card>
      </div>
    ),
