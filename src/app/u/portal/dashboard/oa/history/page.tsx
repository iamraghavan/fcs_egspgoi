...
const RemarkRow = memo(({ 
 ...
 <span className="text-[10px] text-muted-foreground font-mono">{remark.facultySnapshot.facultyID}</span>
 </div>
 </TableCell>
...
export default function IssuedHistoryPage() {
 ...
 <TableRow>
 <TableHead className="text-[11px] font-bold tracking-widest text-cds-text-05">Faculty</TableHead>
 ...
 <CardFooter className="flex items-center justify-between border-t py-3 bg-cds-ui-01/30"><div className="text-[11px] text-muted-foreground font-bold tracking-widest">Page {page} of {totalPages || 1}</div><div className="flex items-center gap-2"><Button variant="outline"size="sm"className="h-8 rounded-none px-4 text-xs"onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}>Previous</Button><Button variant="outline"size="sm"className="h-8 rounded-none px-4 text-xs"onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages}>Next</Button></div></CardFooter>
 </Card>
 ...
 <div>
 <Label className="text-[11px] font-bold tracking-widest text-muted-foreground mb-1 block">
 ...
 <div><Label className="text-[11px] font-bold tracking-widest text-muted-foreground mb-1 block">Administrative Reason</Label><Textarea id="edit-notes"value={editNotes} onChange={(e) => setEditNotes(e.target.value)} className="rounded-none border-0 border-b border-cds-ui-04 bg-cds-ui-01 min-h-[120px] resize-none focus:ring-0 focus:border-b-2"/></div>
 <div><Label className="text-[11px] font-bold tracking-widest text-muted-foreground mb-1 block">Proof Replacement (Optional)</Label><FileUpload onFileSelect={setEditProof} description="Upload a new supporting document"/></div>
 ...
 <div>
 <p className="text-[10px] font-bold text-muted-foreground tracking-widest">Faculty Associate</p>
 <p className="font-bold text-base leading-tight">{selectedRemark.facultySnapshot.name}</p>
 <p className="text-xs font-mono text-muted-foreground">{selectedRemark.facultySnapshot.facultyID}</p>
 </div>
 </div>
 <div className="sm:text-right">
 <p className="text-[10px] font-bold text-muted-foreground tracking-widest">Department</p>
 <p className="font-medium text-cds-text-02">{selectedRemark.facultySnapshot.department}</p>
 </div>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div><p className="text-[10px] font-bold text-muted-foreground tracking-widest mb-1">Violation Title</p><p className="font-semibold leading-tight">{selectedRemark.title}</p></div><div><p className="text-[10px] font-bold text-muted-foreground tracking-widest mb-1">Impact</p><p className={cn("text-2xl font-bold tabular-nums", selectedRemark.type === 'positive' ?"text-cds-support-02":"text-cds-support-01")}>{selectedRemark.type === 'positive' ? '+' : ''}{selectedRemark.points}</p></div></div>
 <div><p className="text-[10px] font-bold text-muted-foreground tracking-widest mb-2">Administrative Reason</p><div className="p-4 bg-cds-ui-01 border-l-4 border-cds-support-01 italic text-cds-text-02 leading-relaxed">{selectedRemark.notes ||"No reason provided."}</div></div>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
 <div>
 <p className="text-[10px] font-bold text-muted-foreground tracking-widest mb-2">Issued By</p>
 ...
 <div>
 <p className="text-[10px] font-bold text-muted-foreground tracking-widest mb-2">Evidence</p>
