...
export default function ManageRemarksPage() {
 ...
 <div>
 <Label className="mb-1"htmlFor="notes">Reason</Label>
 ...
 <div><CardTitle className="text-sm font-semibold">{selectedFaculty.name}</CardTitle><CardDescription className="text-[10px] font-mono">{selectedFaculty.facultyID}</CardDescription></div>
 </CardHeader>
 ...
 <div className="flex flex-col"><span className="font-medium text-cds-text-01 text-[13px]">{r.facultySnapshot.name}</span><span className="text-[10px] text-muted-foreground font-mono">{r.facultySnapshot.facultyID}</span></div>
 </div>
 </TableCell>
 ...
 <div className="text-[11px] text-muted-foreground tracking-widest font-bold">Page {page} of {totalPages || 1}</div>
 ...
 <div>
 <Label className="text-[11px] font-bold tracking-widest text-muted-foreground mb-1 block">Violation Category</Label>
 ...
 <div><Label className="text-[11px] font-bold tracking-widest text-muted-foreground mb-1 block">Administrative Notes</Label><Textarea id="edit-notes"value={editNotes} onChange={(e) => setEditNotes(e.target.value)} className="rounded-none border-0 border-b border-cds-ui-04 bg-cds-ui-01 min-h-[100px] resize-none focus:ring-0 focus:border-b-2"/></div>
 <div><Label className="text-[11px] font-bold tracking-widest text-muted-foreground mb-1 block">Proof Replacement</Label><FileUpload onFileSelect={setEditProof} /></div>
 ...
 <div className="flex items-center gap-4 p-4 bg-cds-ui-01 border border-cds-ui-03"><Avatar className="h-12 w-12 border"><AvatarImage src={selectedRemarkDetails.facultySnapshot.profileImage} /><AvatarFallback>{selectedRemarkDetails.facultySnapshot.name.charAt(0)}</AvatarFallback></Avatar><div><p className="font-bold text-base">{selectedRemarkDetails.facultySnapshot.name}</p><p className="text-xs text-muted-foreground font-mono">{selectedRemarkDetails.facultySnapshot.facultyID}</p></div></div>
 <div className="grid grid-cols-2 gap-6"><div><p className="text-[10px] font-bold text-muted-foreground tracking-widest mb-1">Deduction</p><p className="text-xl font-bold text-cds-support-01 tabular-nums">{selectedRemarkDetails.points}</p></div><div className="text-right"><p className="text-[10px] font-bold text-muted-foreground tracking-widest mb-1">Status</p><div>{getStatusBadge(selectedRemarkDetails.status)}</div></div></div>
 <div><p className="text-[10px] font-bold text-muted-foreground tracking-widest mb-1">Staff Reason</p><blockquote className="mt-1 border-l-2 border-primary/20 pl-4 italic bg-cds-ui-01 p-3 text-cds-text-02 leading-relaxed">{selectedRemarkDetails.notes ||"No reason provided."}</blockquote></div>
